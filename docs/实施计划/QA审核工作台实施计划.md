...
- [x] 编写失败测试，创建一个知识缺失任务，断言列表返回待生成数量和真实任务。
- [x] 编写失败测试，断言详情包含问题摘要、最多三条脱敏代表对话，以及空草稿状态。
- [x] 运行 `./.venv/bin/pytest tests/integration/test_qa_workspace_api.py -q`，确认接口因 `404` 失败。
- [x] 实现任务、草稿和当前版本的查询组装；详情复用问题服务的脱敏样本。
- [x] 重新运行测试，要求查询测试通过。
...
- [x] 编写失败测试，用可覆盖的测试 Provider 生成 QA，连续请求两次并断言 Provider 只调用一次。
- [x] 编写失败测试，断言模型未配置时依赖返回 `503`。
- [x] 运行目标测试，确认当前假模型依赖或缺少接口导致失败。
- [x] 将生成依赖改为读取 `LLM_BASE_URL`、`LLM_API_KEY`、`LLM_MODEL` 的真实 Provider；通过任务定位问题簇并调用现有 `generate_qa_draft`。
- [x] 将模型错误映射为 `502`，业务状态错误映射为 `422`，任务不存在映射为 `404`。
- [x] 重新运行目标测试，要求生成与幂等测试通过。
...
- [x] 编写失败测试，断言缺少业务依据不能批准；完整编辑与依据可创建版本 2，并将任务标记完成。
- [x] 编写失败测试，断言驳回必须填写原因、状态变为 `rejected`、任务取消且写入审计。
- [x] 编写失败测试，断言批准后 JSON/CSV 下载返回附件响应，驳回或待审核草稿不能下载。
- [x] 运行目标测试，确认驳回和下载接口缺失。
- [x] 实现驳回服务和任务状态同步；批准成功后同步任务为 `done`。
- [x] 复用 `export_qa` 的字节内容返回 `Content-Disposition: attachment`，不向页面暴露服务器路径。
- [x] 重新运行目标测试，要求审核与下载测试通过。
...
- [x] 先编写 API 失败测试，断言请求路径、HTTP 方法和批准请求体。
- [x] 先编写页面失败测试，断言待生成状态、六个编辑字段、业务依据、批准与驳回入口。
- [x] 运行 `npm test -- --run src/app/qaApi.test.ts src/features/qa/QAWorkspacePage.test.tsx`，确认模块不存在而失败。
- [x] 实现 API 类型和请求函数；下载使用 Blob 和临时链接触发浏览器保存。
- [x] 实现顶部概况、状态筛选、左侧队列、代表样本、按需生成、草稿编辑、业务依据、批准、驳回和下载。
- [x] 已批准或已驳回时锁定编辑区；错误状态展示具体后端信息并允许重试。
- [x] 将 `/qa/*` 从占位页替换为 `QAWorkspacePage`，并增加桌面与移动端样式。
- [x] 重新运行目标前端测试，要求通过。
...
- [x] 在使用说明增加从知识缺失任务到生成、审核、批准和下载的操作步骤。
- [x] 在文档索引加入本实施计划。
- [x] 运行后端目标测试、现有安全测试集和 Ruff。
- [x] 运行前端全部测试和 `npm run build`。
- [x] 运行 `git diff --check`，确认没有空白错误。
- [x] 检查 Git 状态，只提交本功能相关文件。
