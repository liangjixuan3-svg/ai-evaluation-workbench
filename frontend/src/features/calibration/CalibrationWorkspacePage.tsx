...
export function CalibrationWorkspaceView(props: CalibrationWorkspaceViewProps) {
  ...
  return <section className="operation-page calibration-page">
    ...
    <nav className="calibration-filter" aria-label="校准队列筛选"><span>复核队列</span>{(["pending", "reviewed", "all"] as CalibrationFilter[]).map((status) => <button className={props.status === status ? "active" : ""} key={status} onClick={() => props.onStatusChange(status)}>{filterLabels[status]}</button>)}</nav>
    {workspace.items.length ? <div className="calibration-layout">
      <aside className="calibration-queue" aria-label="校准任务列表">{workspace.items.map((item, index) => <button className={props.selectedId === item.review_id ? "active" : ""} key={item.review_id} onClick={() => props.onSelect(item)}>
      ...
      {detail ? <CalibrationDetailView {...props} detail={detail} canDisagree={canDisagree} isPending={isPending} /> : <div className="calibration-detail-empty">请选择一个校准任务</div>}
...
function CalibrationDetailView(props: CalibrationWorkspaceViewProps & { detail: CalibrationReviewDetail; canDisagree: boolean; isPending: boolean }) {
  const { detail } = props;
  return <main className="calibration-detail">
    ...
    <section className="calibration-section"><SectionTitle number="05" title={props.isPending ? "人工复核" : "人工复核结论"} description={props.isPending ? "请先填写审核人，再确认或修正模型判定" : "已复核案例只读保存，原始模型结果保持不变"} />
      {props.isPending ? <div className="calibration-actions"><label><span>审核人</span><input aria-label="审核人" value={props.actor} placeholder="请输入审核人姓名" onChange={(event) => props.onActorChange(event.target.value)} /></label>{props.showAgreeConfirmation ? <div className="calibration-confirm"><strong>确认认同模型判定？</strong><p>提交后该案例将锁定为只读，无法在此页面修改。</p><div><button className="secondary-button" onClick={() => props.onCancelAgree?.()}>返回检查</button><button className="primary-button" disabled={!props.actor.trim() || Boolean(props.busy)} onClick={props.onAgree}>{props.busy === "agree" ? "正在提交…" : "确认提交"}</button></div></div> : <div className="calibration-action-buttons"><button className="primary-button" disabled={!props.actor.trim() || Boolean(props.busy)} onClick={() => props.onRequestAgree?.()}>认同模型判定</button><button className="secondary-button" disabled={Boolean(props.busy)} onClick={() => props.onShowDisagreeFormChange(!props.showDisagreeForm)}>不认同</button></div>}
...
export function CalibrationWorkspacePage() {
  ...
  async function refresh(preferredId?: string) {
    const response = await getCalibrationWorkspace(status);
    setWorkspace(response);
    const nextId = response.items.find((item) => item.review_id === (preferredId || selectedId))?.review_id ?? response.items[0]?.review_id ?? "";
    setSelectedId(nextId);
    if (nextId) applyDetail(await getCalibrationReview(nextId));
    else setDetail(null);
  }

  useEffect(() => { void ensureTodayCalibrationBatch().then(() => setPrepared(true)).catch((reason) => setError(reason instanceof Error ? reason.message : "今日校准批次准备失败")); }, []);
  useEffect(() => { if (prepared) void refresh().catch((reason) => setError(reason instanceof Error ? reason.message : "校准工作台加载失败")); }, [prepared, status]);

  async function select(item: CalibrationWorkspaceItem) {
    setSelectedId(item.review_id); setBusy("detail"); setError("");
    try { applyDetail(await getCalibrationReview(item.review_id)); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "校准详情加载失败"); }
    finally { setBusy(""); }
  }
  ...
  async function submit(kind: "agree" | "disagree") {
    ...
    try {
      ...
      await refresh(detail.review_id);
