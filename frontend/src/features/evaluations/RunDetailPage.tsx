...
export function RunDetailPage() {
  ...
  return (
    <section className="operation-page run-page">
      <header className="run-header"><div><span className="eyebrow">LIVE EVALUATION</span><h1>评测运行</h1><p>{detail.run_id} · {detail.model}{detail.quality_standard ? ` · ${detail.quality_standard.name} V${detail.quality_standard.version}` : ""}</p></div><span className={`run-state ${detail.stage}`}>{detail.stage === "completed" ? "已完成" : detail.stage === "evaluating" ? "评测中" : detail.stage}</span></header>
